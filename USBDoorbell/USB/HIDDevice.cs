...
namespace USB
{
    ...
    public abstract class HIDDevice : Win32Usb, IDisposable
    {
		...
		/// <summary>Length of output report : device gives us this</summary>
...
//            int error = Marshal.GetLastWin32Error();
			if ( m_hHandle != InvalidHandleValue )	// if the open worked...
			{
				...
				if (HidD_GetPreparsedData(m_hHandle, out lpData))	// get windows to read the device data into an internal buffer
				{
					try
					{
                        ...
                        SafeFileHandle sfHndl = new SafeFileHandle(m_hHandle, true);                                        // updated <<<<< & v
                        m_oFile = new FileStream(sfHndl, FileAccess.Read | FileAccess.Write, m_nInputReportLength, true);	// wrap the file handle in a .Net file stream
        ...
        /// <summary>
        /// Checks for the presence of a device given PID and VID
        /// </summary>
        /// <param name="nVid">Vendor id for device (VID)</param>
        /// <param name="nPid">Product id for device (PID)</param>
        /// <returns>true if present</returns>
        public static bool CheckPresent(int nVid, int nPid)
        {
            string strPath = string.Empty;
            string strSearch = string.Format("vid_{0:x4}&pid_{1:x4}", nVid, nPid); // first, build the path search string
            Guid gHid;
